...
function begin() {
    ...
    global.gridStringInput = document.getElementById("GridStringInput");
    global.moveArea = document.getElementById("MoveArea");
    global.log = document.getElementById("Log");

    setGridString(SOLVED);
    ...
    global.gridStringInput.onfocus = function () {
        this.select();
    }


    global.gridStringInput.onkeypress = function (e) {
        if (!e) e = window.event;
        var keyCode = e.code || e.key;
        if (keyCode == 'Enter') {
            global.bookmark = getGridString();
            resetButton();
        }
    }

    global.moveArea.onchange = function (e) {
        global.cache = global.moveArea.value + " ";
    }
    ...
    if (getGridString() === SOLVED) scramble();
...
function scramble() {
    ...
    global.moveArea.value = "";
    global.moveArea.focus();
    global.bookmark = getGridString();
}
...
function resetButton() {
    if (global.bookmark.length > 0) {
        reset();
        global.cache = global.moveArea.value + "?";
        setGridString(global.bookmark);
        setGrid();
    }
}

function getGridString() {
    return global.gridStringInput.value;
}

function setGridString(value) {
    global.gridStringInput.value = value;
}
...
function setGrid() {
    ...
    for (let r = 0; r < 5; r++) {
        for (let c = 0; c < 5; c++) {
            ...
            cell.row = r + 1;
            cell.col = c + 1;
            cell.actualVal = parseInt(getGridString().substr(actualIndex, 2));
            actualIndex += 2;
            grid.push(cell);
        }
    }
